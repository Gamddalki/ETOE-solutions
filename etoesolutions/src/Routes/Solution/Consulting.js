...
function Consulting() {
  const highlights = useMemo(() => CONSULTING_HIGHLIGHTS, []);
  ...
  return (
    <BodyWrapper pageTitle="컨설팅" pageSubtitle="Technical Consulting">
      <span>
        ...
        시스템을 설계하고 세팅합니다. 그리고 다양한 형태의 맞춤용 서비스 항목을
        기반으로, 고객사의 환경과 업무를 이해하는 전담팀에 의해 고객의 다양한
        요구와 문제점을 신속히 해결하며, 효과적인 기술 전수를 통해 고객만족도
        향상 및 자체 운영 능력을 배양합니다. 또한 당사의 서비스를 계획, 구성하고
        수행, 관리 및 전담팀 운영을 총괄하는 전담 매니저에 의해 일관성 있는
        서비스(Pro-Active Management Services)를 제공합니다. 실무 경험이 풍부한
        컨설턴트가 제공하는 다양한 데이터베이스 컨설팅이 고객의 데이터를
        정확하고 우수한 정보로 만들어 줄 것입니다.
      </span>
      <TwoColumn>
        <ImageBox>
...
export default memo(Consulting);
